...
> Continues **Phase V (web application track, Stages 63–76)** — the
> TWELFTH module of the phase. Adds `std/dom.hls` — a pure-HLS DOM
> tree: an immutable `DomNode` sum type (`Element` / `Void` / `Text`
> / `Raw` / `Comment` / `Doctype` / `Fragment`) plus immutable
> builders, immutable mutators, and a pure serialiser that walks the
> tree into a string. The roadmap's Stage 74 promise — "Server-side
> rendering of HTML. A `dom::Element` tree is built in Halis, then
> serialised to HTML. No client-side JS required (the HTML is
> complete). Hydration is a post-1.0 feature." — is met.

Implements the roadmap's Stage 74 promise:

- **`DomNode` sum type** — `Element(tag, attrs, children)` / `Void(tag,
  attrs)` / `Text(s)` / `Raw(s)` / `Comment(s)` / `Doctype` /
  `Fragment(children)`. The `DomAttr` struct holds a name (validated
  at construction — only `[A-Za-z][A-Za-z0-9_-]*` is accepted) and a
  value (escaped at render time). The tree is heap-allocated; the
  children lists are mutable lists owned by their parent.

- **Strict void-tag invariant** — `dom_element(tag, ...)` PANICS if
  `tag` is one of the 14 HTML5 void elements; `dom_void(tag, ...)`
  panics if `tag` is NOT a void tag. The serialiser would otherwise
  emit invalid HTML5 like `<br></br>`; the panic is the
  type-system-level guarantee that you cannot accidentally emit
  invalid HTML.
...
- **~50 element shorthand builders** — `dom_html` / `dom_head` /
  ...
  inline text (`a`, `em`, `strong`, `code`, `small`, `mark`, `abbr`,
  `b`, `i`, `u`, `sub`, `sup`) + lists (`ul`, `ol`, `li`, `dl`,
  `dt`, `dd`) + tables (`table`, `thead`, `tbody`, `tfoot`, `tr`,
  `th`, `td`, `caption`, `colgroup`, `col`) + forms (`form`,
...
- **Attribute helpers** — `dom_attr(name, value)` + boolean
  attributes `dom_attr_true(name)` / `dom_attr_false(name)` (present
  = emits just the name, no `="value"`; absent = omits entirely —
  the canonical HTML5 form) + shorthand constructors: `dom_class` /
  `dom_id` / `dom_href` / `dom_src` / `dom_alt` / `dom_title_attr`
  / `dom_name_attr` / `dom_value_attr` / `dom_placeholder` /
  `dom_type_attr` / `dom_rel` / `dom_lang` / `dom_charset` /
  `dom_style_attr` / `dom_role` / `dom_data(name, value)` (renders
  as `data-{name}`) / `dom_aria(name, value)` (renders as
  `aria-{name}`).

- **Document helpers** — `dom_html_document(title, body_children)`
  (the minimal `<!DOCTYPE html><html><head><title>...</title></head>
  <body>...</body></html>`) and `dom_html_document_with_head(title,
  head_extras, body_attrs, body_children)` (full control over
  `<head>` and the `<body>`'s attributes).

- **Immutable mutators** (copy-then-return; the receiver is
  unchanged) — `dom_with_attr` / `dom_with_attrs` (for Element +
  Void) and `dom_with_child` / `dom_with_children` (for Element +
  Fragment). The HLS structs are reference-typed, so the convention
  is "copy first, mutate the copy, return the copy" — verified by
  the immutability test.

- **Attribute lookup / replace / remove** (case-insensitive name
  matching) — `dom_attr_get` / `dom_attr_has` / `dom_attr_set`
  (REPLACES the first match in-place, preserves position; or APPENDS
  if not present) / `dom_attr_remove` (removes every match).

- **Introspection** — `dom_node_kind` + the predicates
...
- **Statistics** — `dom_node_count` (total nodes; the root Fragment
  counts itself), `dom_count_by_kind(node, kind)` (the count of
  nodes with a specific kind code), `dom_describe(node)` (an
  indented AST dump — deterministic, byte-identical between
...
- **Serialiser** — `dom_render(node)` walks the tree into a single
  ...
  for trusted HTML). Comment nodes are sanitised against `-->` /
  `--!>` breakout. Doctype always renders to `<!DOCTYPE html>`. Void
  elements are emitted WITHOUT a closing tag. Empty container
  elements are emitted as `<tag></tag>` (NOT self-closing `<tag/>` —
  self-closing is invalid HTML5 for non-foreign elements). Fragment
  is the concatenation of its children. Public aliases:
  `dom_render_children` / `dom_render_attrs` / `dom_render_attr`.
...
- **Inline `<script>` and `<style>` bodies are protected** — the
  ...
  string terminates the element prematurely. `dom_script_inline`
  and `dom_style_inline` rewrite `</script>` and `</style>` to
  `<\/script>` and `<\/style>` (in both lowercase and uppercase
...
**Security model:** DEFAULT-ESCAPE for every `Text` node (OWASP set
`&`, `<`, `>`, `"`, `'`, `/`); attribute values escaped via
`html_escape_attr`; tag/attr names validated (`[A-Za-z][A-Za-z0-9_-]*`);
`Raw` is the explicit opt-in for trusted HTML (NEVER for user input);
comments cannot break out via `-->` or `--!>`; inline script/style
bodies cannot be terminated prematurely by a literal close-tag.
...
(14 tests) covers constants, node constructors, rendering, attribute
helpers, ~50 element shorthand builders, document helpers, immutable
mutators, attribute lookup / replace / remove, statistics, HTTP
integration, XSS hardening (the security model), end-to-end realistic
page, differential stress, and documented negative cases
(panic-level invariants).
...
> Continues **Phase V (web application track, Stages 63–76)** — the
> EIGHTH module of the phase. Adds `std.sse` — a pure-HLS
> implementation of the Server-Sent Events protocol specified by the
> HTML5 / WHATWG Living Standard (§9.2). SSE is the standard wire
> protocol for one-way streaming from a server to a browser over a
> single HTTP/1.1 connection.
>
> Unlike WebSocket (Stage 65, RFC 6455), SSE is UNIDIRECTIONAL: the
> server pushes events; the client cannot send messages back over the
> same connection (it uses a separate HTTP request for any client→server
> traffic). This makes SSE simpler than WebSocket:
>   * It works over plain HTTP (no Upgrade handshake, no framing, no mask)
>   * It auto-reconnects (the browser retries the connection on
>     disconnect, carrying the Last-Event-ID header so the server can
>     resume the stream)
>   * It is natively supported by `EventSource` in JavaScript
>
> Implements the roadmap's Stage 70 promise:
...
>   - `sse_event_new() / sse_event_new_data(data)` — constructors.
...
>     format. Fields are emitted in this order: `id`, `event`, `retry`,
>     `data`, then the trailing blank line `\n\n` that dispatches the
>     event. Multi-line data is split on `\n` and each line is emitted
>     as a separate `data:` field. Empty data still emits `data:\n\n`.
>   - `sse_serialize_or_default(e) -> str` — convenience that panics on
>     validation failure.
>
> - **Parser:**
>   - `sse_parse(s) -> Result[SseEvent, str]` — parse a single event.
>     Liberal: CRLF accepted, field names case-insensitive, comments
>     ignored, unknown fields ignored, multiple `data:` lines joined
>     with `\n`, lines without a `:` ignored.
>   - `sse_parse_stream(s) -> list[SseEvent]` — parse a stream of
>     events. Incomplete trailing data is dropped. Heartbeats (events
>     with no populated fields) are filtered out per the HTML5 spec.
>
> - **Comments & heartbeats:**
>   - `sse_comment(text) -> str` — `: text\n\n`. CR/LF in the text are
>     replaced with spaces (defensive).
>   - `sse_heartbeat() -> str` — the conventional `:ping\n\n`. Servers
>     send this every 15-30 seconds to keep the connection alive through
>     idle-timeout proxies.
>   - `sse_build_heartbeat_block(count) -> str` — `count` consecutive
>     heartbeats.
>
> - **HTTP integration:**
>   - `sse_response() -> HttpResponse` — 200 OK, Content-Type:
>     text/event-stream, Cache-Control: no-cache no-transform (the
>     no-transform is CRITICAL to prevent proxies from gzipping the
>     response, which would break streaming), Connection: keep-alive,
>     X-Accel-Buffering: no (nginx-specific — disables response
>     buffering).
>   - `sse_response_with_events(events) -> HttpResponse` — pre-populated
>     with the given events.
>   - `sse_response_with_heartbeat(events, heartbeat_text) -> HttpResponse`
>     — heartbeat before the events.
>
...
>     validate. Returns Ok(id) if valid, Ok("") if absent, Err(msg) if
>     invalid.
>
> - **Validation:**
>   - `sse_validate_event(e) -> Result[bool, str]` — limits + CR/LF
>     rejection. Rejects id/event_type containing CR or LF, id longer
>     than 256, event_type longer than 64, data larger than 1 MiB,
>     retry_ms outside [1000, 300000] (or non-zero outside the range).
>   - `sse_validate_id(id) / sse_validate_event_type(t)` — standalone
>     validators.
>
> - **Introspection:**
>   - `sse_describe(e) -> str` — human-readable summary. Data truncated
>     to 60 chars.
>
> - **Stream helpers (common patterns):**
>   - `sse_build_message(id, msg)` — a "message" event (default type).
>   - `sse_build_status_update(id, status, retry_ms)` — a "status" event
>     with retry interval.
>   - `sse_build_notification(id, msg)` — a "notification" event.
>   - `sse_build_log_line(line)` — a "log" event. No ID (logs are not
>     resumable).
>   - `sse_build_heartbeat_block(count)` — `count` heartbeats.
>
> Security model: NO-ORIGIN-CHECK + NO-AUTH-CHECK + ID-VALIDATION +
> DATA-INJECTION-PREVENTION are COMPLEMENTARY defenses:
> - **NO ORIGIN CHECK:** SSE is subject to the same-origin policy by
>   default (the browser only allows EventSource from the same origin).
>   Cross-origin requires CORS headers (caller's responsibility).
> - **NO AUTH CHECK:** the caller must authenticate the request before
...
>   server's resume logic. The serialiser rejects CR/LF in id/event_type;
>   the parser does the same. The caller should also validate the
>   Last-Event-ID header before using it.
> - **DATA INJECTION PREVENTION:** the serialiser splits the data on
>   `\n` and emits each line as a separate `data:` field. This prevents
>   a malicious value containing `\n\n` from being interpreted as the
>   end of the event. The serialiser does NOT escape other characters —
>   the data is sent verbatim. If the data is intended for HTML, the
>   caller MUST escape it (e.g. via std.template Stage 69 or std.html
>   Stage 38) before passing it to SSE.
>
> Limitations (deferred to later stages):
> - NO ACTUAL STREAMING I/O: the module produces the wire format and
>   HTTP response, but does NOT push events to a TCP socket. The caller
>   writes the response body to the underlying TCP stream (via std.net
>   Stage 37 or std.http_server Stage 64). A future stage may add a
>   `sse_serve` helper.
> - NO EVENT BUFFERING: the server does not buffer events for
>   disconnected clients. If a client disconnects, events fired during
>   the disconnection are LOST (unless the server stores them externally
>   and uses the Last-Event-ID header to replay).
> - NO MULTI-INSTANCE BROADCAST: no built-in pub/sub across server
>   instances. Use Redis pub/sub or similar.
>
...
> `std.option` (Option[T]), `std.result` (Result[T, E] — for parse
> errors and validation, plus the existing `int_parse` helper). The
> serialiser, parser, and HTTP integration are pure functions of
> immutable inputs; the output is byte-identical between the
> interpreter and the native binary (no Rand, no I/O — fully
> deterministic).
>
> **Acceptance test:** `tests/ok/feat_stage70_sse.hls` (~620 lines, 17
> tests, all PASS) exercises every piece of the API: builder + setters
> (with clamping + immutability), serialiser (basic, multi-line, empty,
> partial), parser (round trip, case-insensitive, CRLF, comments,
> multi-line, stream parsing with heartbeats filtered), comments &
> heartbeats, HTTP integration (response headers, body construction),
> Last-Event-ID (extract + validate), validation (limits + CR/LF
> rejection), introspection, stream helpers, end-to-end counter stream
> with Last-Event-ID resume.
>
> **Demo:** `examples/sse_demo.hls` (~310 lines) demonstrates the
> builder, serialiser, parser, parse_stream (with heartbeats filtered),
> comments & heartbeats, HTTP integration, Last-Event-ID, validation,
> stream helpers, and end-to-end counter stream with Last-Event-ID
> resume.
>
> **Makefile target:** `make sse-acceptance` — runs the demo and the
> acceptance test differentially (interpreter == native for both), then
> prints the feature summary.
...
> Continues **Phase V (web application track, Stages 63–76)** — the
> SIXTH module of the phase. Adds `std.csrf` — a pure-HLS
> implementation of Cross-Site Request Forgery (CSRF) protection,
> building on Stage 66's signed-cookie layer (std.cookie) and Stage
> 67's session storage (std.session). Provides TWO complementary
> defense patterns: the stateless double-submit cookie pattern and
> the stateful synchronizer-token pattern.
>
> Implements the roadmap's Stage 68 promise:
> - **Token generation**: `csrf_generate_token() -> str` (32 random
>   bytes → base64url, 43 chars; 256-bit entropy, matching the session
>   ID format from Stage 67). `csrf_is_valid_token(s)` validates
>   charset + length. `csrf_const_time_eq(a, b)` wraps
>   `cookie_const_time_eq` (Stage 66) for constant-time comparison.
> - **Double-submit cookie pattern** (stateless): the server sets a
>   CSRF token cookie (NON-HttpOnly so JavaScript can read it; Secure;
>   SameSite=Lax). On state-changing requests, the client reads the
>   cookie and submits the SAME value in the `X-CSRF-Token` header.
>   `csrf_double_submit_validate(req, expected_token)` compares the
>   header value, the cookie value, and the server-issued expected
>   token in CONSTANT TIME. An attacker cannot read the cookie
>   cross-origin (Same-Origin Policy), so they cannot forge the header.
> - **Synchronizer-token pattern** (stateful, per-session): the server
>   generates a token bound to a specific session ID and stores it in
>   a `CsrfStore`. `csrf_store_issue(store, session_id, now_ms)`
>   generates and stores a token; `csrf_store_validate(store,
>   session_id, token, now_ms)` looks it up and compares in constant
>   time. Stronger than double-submit because the token never appears
>   in a cookie (immune to cookie-leakage side channels).
> - **HTTP integration**: `csrf_extract_token(req)` checks the
>   `X-CSRF-Token` header first, then the `csrf_token` form field in
>   the request body (for plain HTML forms without JavaScript).
>   `csrf_is_safe_method(method)` returns true for GET/HEAD/OPTIONS
>   (RFC 7231 §4.2.1 safe methods — no token needed).
>   `csrf_require_token(method)` returns true for POST/PUT/PATCH/DELETE.
>   `csrf_check_request(req, store, session_id, now_ms)` is the
>   high-level entry point: returns `Result.Ok(true)` for safe methods
>   or valid tokens, `Result.Err(reason)` otherwise.
> - `csrf_describe_failure(req, reason)` — human-readable failure
>   description for log output and 403 error responses.
>
> Security model: SAMESITE=LAX (Stage 66 session cookie) + DOUBLE-SUBMIT
> (this stage) + SYNC-TOKEN (this stage) are COMPLEMENTARY defenses,
> each protecting against a different threat model. SameSite=Lax blocks
> CSRF on modern browsers (post-2019) but is ignored by older browsers.
> Double-submit blocks CSRF even when SameSite is ignored (attacker
> cannot read the cookie cross-origin). Sync-token is stronger still
> (token never in a cookie). For high-security applications, use BOTH
> patterns (defense in depth).
>
...
> cookie (if HttpOnly is bypassed). CSRF defense assumes the absence
> of XSS — use std.cookie's HttpOnly + a Content Security Policy
> (future stage) to prevent XSS.
>
> Pure-HLS implementation — no new compiler builtins. Layered on
...
>
> Acceptance (`make csrf-acceptance`): 14 sub-tests (is_valid_token,
> generate_token, double_submit_cookie, double_submit_validate_ok,
> double_submit_validate_fail, double_submit_issue_extract, sync_store,
> sync_store_reap_compact, extract_token, method_classification,
> check_request, describe_failure, constants, end_to_end), 2
> differential tests (csrf_demo + feat_stage68_csrf), all green.
...
> Continues **Phase V (web application track, Stages 63–76)** — the
> FIFTH module of the phase. Adds `std.session` — a pure-HLS
> implementation of server-side session storage, building on Stage
> 66's signed-cookie layer (std.cookie). Provides TWO complementary
> backends (in-memory `SessionStore` and file-backed
> `SessionFileStore`) plus the cookie glue to bind them to HTTP.
>
> Implements the roadmap's Stage 67 promise:
> - **Session record**: `Session { id, data, created_ms, expires_ms,
>   last_access_ms }` struct with `session_new`, `session_get`,
>   `session_set`, `session_has`, `session_keys`, `session_is_expired`,
>   `session_touch`, `session_extend`, `session_describe`. The data
>   map is `map[str, str]` (values are str; callers serialise richer
>   types themselves).
> - **ID generation**: `session_generate_id() -> str` (32 random
>   bytes → base64url, 43 chars; 256-bit entropy — OWASP recommended
>   minimum). `session_is_valid_id(s)` validates charset + length,
>   rejecting path-traversal attempts (`/`, `..`), control chars,
>   and non-base64url chars. `session_sanitize_id(s)` is a defensive
>   wrapper that returns `""` for invalid input.
> - **In-memory SessionStore**: `session_store_new(secret,
>   default_ttl_ms)`, `session_store_create` (generate ID + insert),
>   `session_store_get` (returns `Option[Session]`; None if absent or
>   expired; touches `last_access_ms`), `session_store_save`,
>   `session_store_destroy` (marks expired; HLS maps have no
>   `remove()`), `session_store_reap` (count expired), 
>   `session_store_compact` (returns a NEW store with only live
>   sessions — the only way to truly remove expired entries),
>   `session_store_count`.
> - **File-backed SessionFileStore**: one file per session at
>   `<dir>/<id>.sess`. `session_file_store_path(store, id)` composes
>   the path with path-traversal defense (ID validated before use as
>   a filename — `../` and `/` in the ID are impossible).
>   `session_file_store_create/get/save/destroy/reap/count`. File
>   deletion is emulated by writing an empty file (HLS has no
>   file-delete builtin); `session_parse` returns `Err` on empty
>   input, so subsequent loads return `None`.
> - **Serialisation** (text format v1): `session_serialize(s) ->
>   str` renders as `v1\n<id>\n<created>\n<expires>\n<last_access>\n
>   <key>=<value>\n...`. `session_parse(raw) -> Result[Session, str]`
>   is strict: rejects empty input, bad version, truncated header,
>   invalid session ID, and non-numeric timestamps. Values may
>   contain `=` (the parser splits on the FIRST `=`); newlines are
>   escaped as `\n` (the literal sequence) before serialisation.
> - **Cookie glue**: `session_issue_cookie(session, secret, now_ms) ->
>   SetCookie` builds a signed (HMAC-SHA1 via std.cookie Stage 66),
>   Secure, HttpOnly, SameSite=Lax cookie named "sid" with Max-Age
>   computed from the session's expiry. `session_attach_cookie` appends
>   it to a response. `session_destroy_cookie(secret)` builds a
>   Max-Age=0 logout cookie. `session_id_from_request(req, secret) ->
>   Option[str]` parses the Cookie header, verifies the signature,
>   and extracts the original ID. `session_load_from_request` and
>   `session_load_from_request_file` are convenience wrappers that
>   look up the session in the in-memory or file-backed store.
>
> Security model: SECURE + HTTP-ONLY + SAMESITE=LAX + SIGNED are
> COMPLEMENTARY defenses (passive sniffing / XSS-based session
> hijacking / CSRF / server-side state forgery). The session ID is
> an opaque 256-bit random capability token — possession of the ID
> is sufficient to act as the session. The ID is delivered via a
> signed cookie so the server can detect forgery EVEN IF an attacker
> can write the cookie (via XSS that bypassed HttpOnly, or via a
> subdomain sharing the parent Domain).
>
> Pure-HLS implementation — no new compiler builtins. Layered on
...
> fs_read_dir).
>
> Acceptance (`make session-acceptance`): 15 sub-tests (is_valid_id,
> generate_id, session_record, expiry, describe, in_memory_store,
> in_memory_store_expiry, serialise_parse, parse_errors, cookie_round_trip,
> cookie_rejection, cookie_attach_destroy, file_store,
> file_store_path_safety, constants), 2 differential tests (session_demo
> + feat_stage67_session), all green. Bootstrap self-compilation remains
> deterministic.
...
> expansion) — implementing a CONSTANT-MEMORY streaming JSON parser
> that produces one token at a time and never holds the whole
> document in memory. **No new compiler builtins** — the entire
> module is pure HLS, with the caller driving the feed.
>
> Useful for parsing multi-GB JSON logs, NDJSON streams, or any
> JSON source that's too large to fit in RAM. The existing
> `std.json` document parser (Stage 10-beta, v0.6.0-alpha) remains
> for small inputs where the caller wants a `JsonValue` tree.
>
...
> bytes for the second UTF-8 byte. The streaming parser ships the
> correct formula; the same fix is noted as a future backport to
> `std.json`.

### Added — Stage 40 (v0.59.0-alpha)

- **`std/json_stream.hls`** library module with:
  - **`JsonToken` enum** — 10 variants covering all JSON value
    ...
    - `Key(str)` — an object member name (the string BEFORE
      the `:`). Distinguished from `String` by the reader's
      state machine (the first string token after `ObjectStart`
      or `,` inside an object is emitted as a Key).
    - `String(str)` — a string value (after `:` or in an array).
    - `Int(int)` — integer in int64 range (matches std.json:
      accepts INT64_MIN, rejects INT64_MAX+1).
    - `Float(float)` — number with `.` or `e` / `E`.
    - `Bool(bool)` — `true` or `false`.
    - `Null` — `null`.
  - **`JsonReader` struct** — the streaming reader:
    - `buf` (internal byte buffer), `pos` (current read offset),
      `err` (error state, "" on success), `finished` (true after
      `json_reader_finish`), `state` (parser state machine),
      `container_stack` (push/pop on `{` / `[`), `max_token_size`
      (default 64 KiB).
    - Mutable (HLS structs have reference semantics — SPEC §2.1 —
      so mutations propagate to the caller, matching the
      `JsonParser` pattern in `std.json`).
  - **State machine** — 6 states: `TOP` (before/after root
    value), `OBJECT_KEY` (after `{` or `,` in object; expecting
    a Key or `}`), `OBJECT_COLON` (after Key emit; expecting
    `:`), `OBJECT_VAL` (after `:`; expecting a value),
    `ARRAY_VAL` (after `[` or `,` in array; expecting a value
    or `]`), `DONE` (after root value at top level; only
    trailing whitespace allowed). The `container_stack` is
    pushed/popped on `{` / `[` / `}` / `]`; the state is
    restored to the parent container's state when a container
    closes.
  - **API**:
    - `json_reader_new() -> JsonReader` — fresh reader with
      default settings.
    - `json_reader_set_max_token_size(r, n) -> void` — tune the
      per-token byte limit (default 64 KiB, max 16 MiB).
    - `json_reader_feed(r, bytes) -> Result[int, str]` — append
      bytes. Returns `Err` if the buffer would exceed the 16 MiB
      hard cap.
    - `json_reader_finish(r) -> Result[int, str]` — signal end
      of input. After this, `feed` returns `Err`.
    - `json_reader_next_token(r) -> Result[Option[JsonToken],
      str]` — returns:
      - `Ok(Some(token))` — a token was produced.
      - `Ok(None)` — need more input OR cleanly at EOF with
        state DONE.
      - `Err(msg)` — parse error (terminal state).
    - `json_reader_drain(r, max_tokens) -> Result[list[JsonToken],
      str]` — convenience: call `next_token` until `None` or
      cap. NOT for production use on huge documents (defeats
      the streaming property).
    - `json_reader_parse_all(src) -> Result[list[JsonToken],
      str]` — one-shot: feed whole src, finish, drain. Use only
      when the whole document fits in memory AND the caller wants
      the token list.
  - **String parsing** — handles all 9 escape sequences (`\n`,
    ...
    10-beta's behavior, with the 4-byte UTF-8 bug fixed).
  - **Number parsing** — int (in int64 range) or float (with
    `.`, `e`, `E`, `+`, `-`). RFC 8259 §6 leading-zero
    rejection (`0123` is invalid).
  - **Token-kind predicates** — 10 helpers for pattern-matching
    on `JsonToken`: `json_token_is_object_start`,
    `_is_object_end`, `_is_array_start`, `_is_array_end`,
    `_is_key`, `_is_string`, `_is_int`, `_is_float`,
    `_is_bool`, `_is_null`.
  - **`json_token_kind_name(t) -> str`** — diagnostic name
    for a token kind (for logging / error messages).
- **`examples/json_stream_demo.hls`** and
  **`tests/ok/feat_stage40_json_stream.hls`** exercising:
  - One-shot parse (10 tokens for `{"name":"alice","age":30,...}`).
  - Incremental parse (byte-by-byte feed; 8 tokens for
    `{"a":1,"b":2,"c":3}`).
  - Chunk-boundary independence (1/4/16/64/256-byte chunks
    produce same token sequence).
  - Nested containers (13 tokens for
    `{"outer":{"inner":[1,{"x":2}]}}`).
  - String escapes (8 escape sequences decoded).
  - Surrogate pair decoding (`\uD83D\uDE00` -> 4-byte UTF-8
    [0xF0, 0x9F, 0x98, 0x80]).
  - Number edge cases (13 values: int, float, negative,
    exponent).
  - Integer range (INT64_MAX and INT64_MIN accepted; INT64_MAX+1,
    INT64_MIN-1, 19-digit values rejected).
  - Empty containers (`{}` -> 2 tokens, `[]` -> 2 tokens).
  - Trailing data after root value (3 cases rejected).
  - Empty input (2 cases rejected).
  - Unbalanced containers (4 cases rejected).
  - Invalid escapes (`\x`, `\u12`, `\uZZZZ` rejected).
  - max_token_size limit (20-byte string rejected with limit 16).
  - Nesting depth limit (1100-deep array rejected with limit 1024).
  - NDJSON stream (3 records, 24 tokens, msg values extracted).
  - Large document (1000-element array, 4002 tokens, 256-byte
    chunks).
  - Keyword truncation (`tru`, `fals`, `nul` rejected at EOF).
  - Leading-zero rejection (`0123`, `00` rejected; `0` and `0.5`
    accepted).
  - Token-kind predicates (10 predicates verified).
  - Kind names (10 token kinds have correct diagnostic names).
  - parse_all convenience wrapper.
- **ROADMAP** updated: Stage 40 marked ✅ complete.

### Changed — Stage 40 (v0.59.0-alpha)

- **`Makefile`** — added the `json-stream-acceptance` target
  (mirrors `http2-acceptance`'s structure; pure-HLS module so
  the link command is identical). Updated `.PHONY` to include
  the new target.
- **No compiler changes** — Stage 40 is pure HLS, layered on
  `std.result`, `std.option`, and `std.str`. The
  `boot/checker.py`, `boot/interp.py`, `src/hlc.hls`, and
  `tools/llvm_emit.py` files are unchanged from v0.58.0-alpha.
  This is the third consecutive roadmap stage that adds a
  stdlib module without touching the compiler — the language's
  self-hosting maturity is sufficient for pure-HLS protocol
  implementations and parsers.

### Fixed — Stage 40 (v0.59.0-alpha)

- **4-byte UTF-8 encoding bug** (in `std/json_stream.hls` only;
  `std/json.hls` retains the original buggy behavior for
  backwards compatibility, to be fixed in a future deep-scan):
  the formula for encoding codepoints in [U+10000, U+10FFFF]
  (4-byte UTF-8) used `c = cp - 65536` instead of `cp` directly.
  This produced incorrect bytes for the second UTF-8 byte (off
  by 1 in the bit-16 position) — e.g. U+1F600 (grinning face
  emoji) was encoded as [0xF0, 0x8F, 0x98, 0x80] instead of the
  correct [0xF0, 0x9F, 0x98, 0x80]. The bug only manifested for
  surrogate pairs encoding emoji and rare CJK extension
  characters; ASCII and BMP codepoints (U+0000..U+FFFF) were
  unaffected. The streaming parser ships the correct formula;
  the same fix is noted as a future backport to `std.json`'s
  `jsonp_push_utf8` (one-line change).

### Security — Stage 40 (v0.59.0-alpha)
...
- **Maximum token size** (default 64 KiB): defends against a
  ...
  `json_reader_set_max_token_size`. The hard cap is 16 MiB
  (defends against a caller that feeds huge chunks without
  bounding max_token_size).
- **Maximum nesting depth** (default 1024): defends against a
  ...
  bpo-29381; CVE-2017-20018 in some C JSON parsers is the
  analogous advisory).
- **Integer range check**: integers are checked against int64
  range (matches Stage 10-beta's `json_parse_result` behavior —
  out-of-range integers return `Err` instead of panicking with
  a raw "integer too large" message).
...
- **Surrogate pair decoding**: `\uXXXX\uYYYY` pairs ARE decoded
  (high surrogate followed by low surrogate combines into a
  single codepoint, emitted as UTF-8 bytes). High surrogate
  not followed by low surrogate -> `Err`; low surrogate without
  preceding high surrogate -> `Err`.
- **Constant memory** (the headline property): the reader's
  buffer high-water mark is `max_token_size` + the largest
  single `feed` chunk (default 64 KiB + 4 KiB = ~68 KiB). A
  1 GB JSON document parses in ~70 KiB of reader memory
  regardless of document size — the parser is constant-memory
  in the document size, linear in the chunk size + max token
  size.

### Differential parity (interpreter == native) — Stage 40

- The parser is a pure state machine over `str` bytes. The
  output token sequence is byte-identical between the
  interpreter and the native binary (no I/O — the caller
  drives the feed).
- The acceptance test (`make json-stream-acceptance`) verifies
  differential parity on:
  - Simple object (6 tokens for `{"name":"alice","age":30}`).
  - Simple array (8 tokens for
    `[1,2,3,"four",true,null]`).
  - Incremental parse (byte-by-byte feed; same 6 tokens as
    one-shot).
  - Chunk-boundary independence (1/4/16/64/256-byte chunks
    produce same 17 tokens for a complex nested document).
  - Nested containers (13 tokens for
    `{"outer":{"inner":[1,{"x":2}]}}`).
  - String escapes (8 escape sequences decoded: `\n`, `\t`,
    `\r`, `\"`, `\\`, `\/`, `\b`, `\f`).
  - Surrogate pair decoding (`\uD83D\uDE00` -> 4-byte UTF-8
    [0xF0, 0x9F, 0x98, 0x80] — the fixed formula).
  - Number edge cases (13 values: int, float, negative,
    exponent).
  - Integer range (INT64_MAX and INT64_MIN accepted;
    INT64_MAX+1, INT64_MIN-1, 19-digit values rejected).
  - Empty containers (`{}` -> 2 tokens, `[]` -> 2 tokens).
  - Trailing data after root value (3 cases rejected).
  - Empty input (2 cases rejected).
  - Unbalanced containers (4 cases rejected).
  - Invalid escapes (`\x`, `\u12`, `\uZZZZ` rejected).
  - max_token_size limit (20-byte string rejected with limit 16).
  - Nesting depth limit (1100-deep array rejected with limit
    1024).
  - NDJSON stream (3 records, 24 tokens, msg values extracted).
  - Large document (1000-element array, 4002 tokens, 256-byte
    chunks).
  - Keyword truncation (`tru`, `fals`, `nul` rejected at EOF).
  - Leading-zero rejection (`0123`, `00` rejected; `0` and
    `0.5` accepted).
  - Token-kind predicates (10 predicates verified).
  - Kind names (10 token kinds have correct diagnostic names).
  - parse_all convenience wrapper (`[1,2,3]` -> 5 tokens).
  - feed after finish (rejected).

### Stage 40 limitations (deferred to later stages)

- **No UTF-8 validation** in string tokens — the byte sequence
  is returned as-is. The caller is responsible for validating
  UTF-8 if the consuming application requires it. A future
  Stage 50+ `std.str` will add a `str_validate_utf8` helper.
- **No chunked string API** — a single string token that
  exceeds `max_token_size` is rejected with `Err`. A future
  stage may add a "string chunk" API where a long string is
  emitted as multiple String tokens with a continuation flag.
- **`std.json`'s 4-byte UTF-8 bug is NOT fixed** (deliberate —
  changing `std.json` would break differential parity for
  Stage 10-beta's acceptance test until a regression test is
  added). The fix is noted as a future backport; for now, code
  that needs correct UTF-8 for codepoints above U+FFFF should
  use `std.json_stream` instead of `std.json`.
...
> [operands], [options(...)])` — the foundation for low-level
> kernel, driver, and runtime code that needs to emit raw CPU
> instructions (port I/O, CPUID, RDTSC, interrupt enable/disable,
> `hlt`, etc.) without dropping out of the HLS type system. The
> statement is lowered to GCC's extended-asm syntax
> (`__asm__ __volatile__("..." : outputs : inputs : clobbers)`),
> with the template `{N}` placeholders rewritten to `%N` and the
> direction prefixes (`=`, `+`, `=&`) applied automatically.
>
> Four operand directions are supported: `in(constraint) expr`
> (read-only input), `out(constraint) lvalue` (write-only output),
> `inout(constraint) lvalue` (read-write output), and
> `late_out(constraint) lvalue` (early-clobber output). Constraints
> are either bare register classes (`reg`, `mem`, `imm`) or
> specific-register string forms (`"eax"`, `"edx"`, ...). Six
> options are supported: `pure` (drops `__volatile__`), `nomem`
> (no "memory" clobber), `preserves_flags` (no "cc" clobber),
> `noreturn` (emits `__builtin_unreachable()`), `nostack` (no-op
> for source compatibility), `attsyntax` (no-op on x86-64).
>
> The acceptance criterion from the roadmap is satisfied: a
> typesafe `inb(port) -> int` helper compiles to a SINGLE `in`
> instruction with no compiler-generated memory accesses
> (verified by objdump). The `make asm-acceptance` gate is GREEN
> and **17 new tests** in `tests/run_tests.sh` section 18 PASS.
> Bootstrap is deterministic with the Stage 27 changes; the full
> local suite is **810 PASS / 0 FAIL** (was 793, +17 Stage 27
> tests).

### Added — `asm` keyword (lexer)

- `boot/lexer.py` `KEYWORDS`: added `"asm"`. The `!` that follows
  in the `asm!` statement form is the existing single-char `!` sym
  (the unary-not operator) — no new lexer symbol is needed.
- `src/hlc.hls` `is_keyword`: same addition for parity.

### Added — `parse_asm_stmt` (parser)

- `boot/parser.py`: new `parse_asm_stmt` method. Parses the grammar
  `asm! ( string_lit (, operand)* (, options(...))? )` with operands
  `in(constraint) expr`, `out(constraint) lvalue`,
  `inout(constraint) lvalue`, `late_out(constraint) lvalue`.
  The constraint is either an IDENT (`reg`, `mem`, `imm`) or a
  string literal (`"eax"`, `"edx"`, ...). The `pure` keyword
  (already reserved by Stage 9-beta) is accepted in the options
  list as a special case.
- `boot/parser.py`: new `at_ident(name)` helper — checks whether
  the current token is an identifier with the given name (used to
  recognise `options` / `out` / `inout` / `late_out` — these are
  NOT keywords; only `in` is, since it's also the for-loop
  keyword).
- `src/hlc.hls`: mirrored `parse_asm_stmt` for the self-hosted
  compiler. Both parsers store the AST on a new
  `StmtN.kind == "asm"` node with three new fields:
  `asm_template` (str), `asm_operands` (list[AsmOperand]),
  `asm_options` (list[str]).

### Added — `AsmOperand` struct + `StmtN` fields

- `src/hlc.hls`: new `AsmOperand` struct records one asm! operand:
...
- `src/hlc.hls`: three new fields on `StmtN` (`asm_template`,
  `asm_operands`, `asm_options`). The `nx_stmt` constructor
  initialises them to `""`, `[]`, `[]` so non-asm statements are
  unaffected.

### Added — `check_asm` (checker)

- `boot/checker.py`: new `check_asm` method. Validates:
...
- `src/hlc.hls`: mirrored `check_asm` for the self-hosted compiler.
- `src/hlc.hls`: `all_return` is updated so a `noreturn` asm! as
  the last statement of a function counts as a return-on-all-paths
  (the codegen emits `__builtin_unreachable()` after the asm).
- `src/hlc.hls`: new `parse_placeholder_int` and
  `list_contains_str` helpers used by `check_asm`.

### Added — `gen_asm` (codegen)

- `src/hlc.hls`: new `gen_asm` function — lowers the HLS `asm!` to
  GCC's extended-asm syntax:
  ```
  ...
  `late_out` -> `=&`. The default clobber list is `"cc", "memory"`;
  `nomem` suppresses `"memory"`, `preserves_flags` suppresses
  `"cc"`. `pure` drops `__volatile__` (gcc may elide a pure asm).
  `noreturn` emits `__builtin_unreachable();` after the asm.
- `src/hlc.hls`: new `translate_asm_constraint` function — converts
  an HLS asm! constraint to the GCC constraint string. Bare
  register classes: `reg` -> `r`, `mem` -> `m`, `imm` -> `i`.
  Specific-register string forms: `eax`/`rax` -> `a`, `ebx`/`rbx`
  -> `b`, `ecx`/`rcx` -> `c`, `edx`/`rdx` -> `d`, `esi`/`rsi` ->
  `S`, `edi`/`rdi` -> `D`. Unknown string-form constraints pass
  through verbatim (the user takes responsibility for the
  validity).
- `src/hlc.hls`: new `translate_asm_template` function — rewrites
  the HLS asm! template (which uses `{N}` placeholders) to the
  GCC form (which uses `%N` positional placeholders, `%%` for a
  literal `%`, and `{{` / `}}` for literal braces). GCC's template
  modifiers (`%b0`, `%w1`, `%k0`, `%q0`, `%%`) pass through
  verbatim — the user can write either `{0}` (HLS placeholder) or
  `%b0` (GCC modifier) in the template.
- `src/hlc.hls`: new `c_str_literal` function — produces a C
  string literal (just the quoted form, WITHOUT the existing
  `c_str_lit`'s `hl_cstr("...")` wrapper — the GCC extended-asm
  syntax expects a bare string token, not a function-call
  expression). The existing `c_str_lit` is unchanged (it's still
  used by the regular string-literal codegen path).
- `src/hlc.hls`: new `join_str_list` helper (joins a `list[str]`
  with a separator — used by the asm codegen to build the
  outputs/inputs/clobbers lists).

### Added — `boot/interp.py` asm! handler

- `boot/interp.py`: new `k == "asm"` case in `exec_stmt`. Raises a
  clean `HLPanic` ("asm! cannot be executed by the boot
  interpreter — use the native compiler `hlc`") when an `asm!` is
  executed. Programs that DECLARE `asm!` blocks but do NOT execute
  them at interpreter runtime (e.g. an `asm!` inside a kernel IRQ
  handler never called from `main`) run fine — matching the
  kernel_irq_demo.hls pattern from Stage 28.

### Added — `examples/asm_demo.hls`

- A complete demo exercising all five asm! forms:
  - `asm!("nop")` (bare)
  ...
  - `asm!("mov {1}, {0}", in(reg) x, out(reg) y, options(pure,
    nomem, preserves_flags))` (pure)
- The `inb()` helper compiles to a single `in %dx, %al`
  instruction (verified by `objdump -d` on the produced .o file —
  no compiler-generated memory accesses thanks to the `nomem`
  option).

### Added — `tests/ok/feat_stage27_asm.hls`

- The Stage 27 acceptance test, declaring all six asm!-containing
  helpers (`asm_nop`, `asm_set_zero`, `asm_add`, `asm_inb`,
  `asm_halt`, `asm_pure`). Verified by the boot interpreter
  (smoke), the boot checker (type check), the self-hosted hlc
  (compile), gcc (link), and objdump (single `in` instruction).

### Added — `make asm-acceptance` + `tests/run_tests.sh` section 18

- `Makefile`: new `asm-acceptance` target — verifies the full
  Stage 27 acceptance criterion: HLS parses, C source has
  `__asm__`, default clobber list is present, noreturn emits
  `__builtin_unreachable()`, pure drops `__volatile__`, C compiles
  cleanly with gcc -O2 -Werror, and `inb()` compiles to a single
  `in` instruction (verified by objdump on the produced .o file).
- `Makefile`: new `asm-attrs` target — prints the asm! templates
  and operands of every function in the given HLS file.
- `tests/run_tests.sh` section 18: 17 new tests covering all the
  above plus negative tests (out-of-range `{N}` placeholder,
  pure-with-no-outputs, noreturn-with-outputs, immutable out
  operand).

### Fixed — deep-scan (post-Stage 27)

- `boot/parser.py`: removed a stray stub `while not self.at_sym("}")`
  loop in `parse_asm_stmt`'s options handler (a leftover from
  early development that never executed but was confusing to read).
- `src/hlc.hls` `gen_asm`: declared `volatile_kw` as `mut` so the
  `pure` option can reassign it (was previously a `let` that
  caused a compile error in the self-hosted checker).
- `src/hlc.hls` `translate_asm_constraint`: declared `prefix` and
  `body` as `mut` so they can be reassigned by the direction and
  register-class branches.

## [v0.49.0-alpha] — Stage 26: RISC-V 64 backend (RVV + bare-metal)

> Stage 26 adds the RISC-V 64 backend as the foundation for OS-
> development work (Stages 77–96). Two RISC-V 64 targets are
> supported: `riscv64gc-unknown-linux-gnu` (full Linux user-mode,
> RV64GC: IMAFDC) and `riscv64-unknown-none` (bare-metal, RV64IMAC,
> no OS, no libc). The RISC-V Vector (RVV) extension `V` is engaged
> via `--target-feature rvv`, which emits `<riscv_vector.h>`
> intrinsics (`__riscv_vadd_vv_i32m1`, `__riscv_vsub_vv_i32m1`,
> `__riscv_vmul_vv_i32m1`, `__riscv_vmin_vv_i32m1`,
> `__riscv_vmax_vv_i32m1` for i32x4; `__riscv_vadd_vv_f64m1`,
> `__riscv_vsub_vv_f64m1`, `__riscv_vmul_vv_f64m1` for f64x2) under a
> `#if defined(__riscv) && defined(__riscv_v)` guard, with the same
> portable scalar fallback as the x86 (SSE4.2/AVX2) and AArch64
> (NEON) paths. The same C source compiles on every host.
> `make riscv-acceptance` (RVV codegen) and `make riscv-bare-
> acceptance` (freestanding compile) are GREEN; bootstrap
> deterministic with `--target-feature rvv`; full local suite
> **793 PASS / 0 FAIL** (was 779, +14 Stage 26 tests).

### Added — RISC-V Vector (RVV) intrinsic emission (`src/hlc.hls`)

- New `simd_helpers_rvv` field on the `Ctx` struct (holds the C
  lines of the RVV intrinsic bodies, populated when
  `--target-feature rvv` is passed).
- New `simd_emit_rvv_i32x4_ew(ctx, name, intrinsic)` function —
  emits one elementwise i32x4 kernel using RVV intrinsics. The body
  loads `x` and `y` from their `{i64 a, i64 b}` layout into
  `vint32m1_t` registers via `__riscv_vle32_v_i32m1`, applies the
  intrinsic with `vl=4` (4 i32 lanes at LMUL=1, VLEN≥128), and
  stores the result back via `__riscv_vse32_v_i32m1`. The intrinsic
  name (`__riscv_vadd_vv_i32m1`, `__riscv_vsub_vv_i32m1`,
  `__riscv_vmul_vv_i32m1`, `__riscv_vmin_vv_i32m1`,
  `__riscv_vmax_vv_i32m1`) is passed in.
- New `simd_emit_rvv_f64x2_ew(ctx, name, intrinsic)` function —
  emits one elementwise f64x2 kernel using RVV intrinsics. Loads via
  `__riscv_vle64_v_f64m1`, stores via `__riscv_vse64_v_f64m1`. `vl=2`
  (2 f64 lanes at LMUL=1, VLEN≥128).
- Modified `simd_emit_helper(ctx, name)` — each dispatch entry now
  also checks `ctx.target_feature == "rvv"` and calls the RVV
  emitter (instead of the x86 or NEON emitter) when true. The same
  scalar fallback is always emitted.
- Modified `simd_helper_lines(ctx)` — new
  `else if ctx.target_feature == "rvv"` branch emits
  `#if defined(__riscv) && defined(__riscv_v)` /
  `#include <riscv_vector.h>` / `simd_helpers_rvv` /
  `simd_helpers_plain` / `#else` / `simd_helpers_alt` /
  `simd_helpers_plain` / `#endif`.
- Modified `simd_detect_native_feature()` — new probe:
  `simd_cpu_supports("rvv")` returns true on a RISC-V 64 host whose
  `/proc/cpuinfo` `isa:` line contains the V extension. Returns
  `rvv` when true.
- Modified `--target-feature` CLI parsing — accepts `rvv` (and
  `+rvv`) in addition to `sse4.2`, `avx2`, `neon`, `native`. The
  usage string is updated to
  `--target-feature sse4.2|avx2|neon|rvv|native`.

### Added — RISC-V 64 cross-compilation targets (`tools/hlcross.py`)

- New target `riscv64gc-unknown-linux-gnu` (Linux user-mode,
  RV64GC: IMAFDC). `arch=riscv64`, `os=linux`, `abi=gnu`,
  `binary_format=ELF riscv64 (Little Endian, RV64GC)`,
  `march=rv64gc`, `mabi=lp64d`. When `--target-feature rvv` is
  given, `march` is promoted to `rv64gcv` (G + C + V).
- `riscv64-unknown-linux-gnu` is an alias for the same target.
- New target `riscv64-unknown-none` (bare-metal, RV64IMAC).
  `arch=riscv64`, `os=none`, `abi=none`, `binary_format=ELF
  riscv64 (Little Endian, RV64IMAC, freestanding)`, `march=rv64imac`,
  `mabi=lp64`, `freestanding=True` (causes `cross_compile` to add
  `-nostdlib -nostartfiles -ffreestanding` to the linker command),
  `link_libs=[]` (no libc, no libm, no pthread).
- New `march`/`mabi`/`freestanding` fields on RISC-V targets. The
  `cross_compile` function appends `-march=<march> -mabi=<mabi>` to
  the linker command for RISC-V targets, and
  `-nostdlib -nostartfiles -ffreestanding` when `freestanding=True`.
- New cross-linker detection:
  - Linux user-mode: `riscv64-linux-gnu-gcc`,
    `riscv64-linux-gnu-gcc-13`, `riscv64-linux-gnu-gcc-12`,
    `riscv64-linux-gnu-cc`, `riscv64-unknown-linux-gnu-gcc`.
  - Bare-metal: `riscv64-unknown-elf-gcc`, `riscv64-elf-gcc`,
...
- Modified `host_triple()` — recognises RISC-V 64 hosts
  (`riscv64`, `rv64`, `riscv`) and returns
  `riscv64gc-unknown-linux-gnu`.
- Modified `cmd_list_targets()` — prints `march`/`mabi` for targets
  that have them; prints the freestanding flag note for targets with
  `freestanding=True`.
- Modified `--target-feature` CLI flag — accepts `rvv` in addition
  to `neon`, `sse4.2`, `avx2`, `native`.

### Added — `tools/hlriscv.py`

- New helper that wraps `hlcross.cross_compile` with the right
  RVV + `-march`/`-mabi` flags. Defaults to
  `--target riscv64gc-unknown-linux-gnu --target-feature rvv`.
- `--list-targets` prints the supported RISC-V target triples + the
  RVV feature + the bare-metal flags.
- For the bare-metal target (`riscv64-unknown-none`), the
  `--target-feature rvv` is silently stripped (rv64imac has no V
  extension).

### Modified — `boot/boot.py` + `boot/interp.py`

- `boot/boot.py` `--target-feature` parsing — accepts `rvv` (and
  `+rvv`). The `--target-feature native` auto-detect now also
  probes `_cpu_supports("rvv")` and resolves to `rvv` on a RISC-V
  64 host with the V extension.
- `boot/interp.py` `_cpu_supports(feat)` — new `feat == "rvv"`
  branch: returns `True` on a RISC-V 64 host whose `/proc/cpuinfo`
  `isa:` line contains the V extension. Returns `False` on
  non-RISC-V hosts (best-effort runtime probe).
...
- `make riscv-acceptance` — the Stage 26 RVV codegen acceptance
  gate. Verifies that: (a) the C source for `simd_bench.hls`
  contains RVV intrinsics; (b) it includes `<riscv_vector.h>`;
  (c) it has the `__riscv && __riscv_v` guard; (d) the C source
  compiles + runs on x86_64 via the scalar fallback; (e) on a
  RISC-V 64 host, the runtime benchmark compares RVV vs baseline
  (SKIPped on non-RISC-V hosts — the static checks still pass).
...
- `make riscv-bare-acceptance` — the Stage 26 bare-metal
  acceptance gate. Verifies that: (a) the C source was produced;
  (b) the C source compiles under `-ffreestanding -nostdlib -c`
  (the user's code is freestanding-compatible); (c) when a RISC-V
  cross-linker is available, the binary's ELF header identifies it
  as RISC-V 64. The full "zero libc references" binary requires
  the freestanding runtime (Stage 77+ `#![freestanding]` mode);
  Stage 26 wires up the cross-compilation pipeline + verifies the
  C source is freestanding-compatible.
- `make riscv-list-targets` — print the RISC-V target + feature
  set.

### Added — examples + tests

- `examples/riscv_demo.hls` — a demo program for both RISC-V
  targets: `fib_iter` (pure integer Fibonacci), `riscv_sum_range`
  (pure integer sum), `riscv_rvv_demo` (exercises the std.simd
  kernels that lower to RVV intrinsics under `--target-feature
  rvv`). The `main` function prints the results (Linux user-mode
  entry). The bare-metal target uses the same C source — the
  freestanding compile succeeds because the C source compiles
  under `-ffreestanding -c`.
- `tests/ok/feat_stage26_riscv.hls` — the Stage 26 ok-test:
  exercises `fib_iter`, `riscv_sum_range`, and the std.simd kernels
  (add/sub/mul/min/max for i32x4) on the portable path AND under
  `--target-feature rvv` (which sets `has_feature("rvv")=true`).
- 14 new tests in `tests/run_tests.sh` section 16: RVV intrinsics
  ...
  to true under `--target-feature rvv`.

### Test suite

- **793/793 PASS** (was 779/779 in v0.48.2-alpha). Bootstrap still
  deterministic with `--target-feature rvv` (the self-hosted
  compiler doesn't use `std.simd`, so the RVV code paths don't
  affect bootstrap output).
